...
export default {
    ...
    created() {
        // 如果路由中有课程ID，则预设课程
        const routeCourseId = this.$route.params.id
        if (routeCourseId) {
            this.homeworkForm.courseId = parseInt(routeCourseId)
        }
        ...
        async handleSubmit() {
            try {
                ...
                const submitData = {
                    ...this.homeworkForm,
                    ...this.publishForm,
                    status: this.publishForm.publishType === 1 ? 2 : 1,
                ...
                delete submitData.publishType

                await homeworkCreate(this.homeworkForm.courseId, submitData)

                this.$message.success(this.publishForm.publishType === 1 ? '作业发布成功' : '作业保存成功')
                this.$router.push(`/teacher/course/${this.homeworkForm.courseId}/homework`)
